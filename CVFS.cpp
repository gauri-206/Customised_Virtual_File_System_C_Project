...
{
    int i = 0;      // int i = 3    (First 3 inodes are reserved [IN OUT ERR])
    PINODE temp = head;
    ...
    UFDTArr[i].ptrfiletable->ptrinode->permissiom = permission;
...
int ReadFile(int fd, char *arr, int isize)
{
    ...
    if(UFDTArr[fd].ptrfiletable->ptrinode->permissiom != READ && UFDTArr[fd].ptrfiletable->ptrinode->permissiom != READ+WRITE)      return-2;
...
int WriteFile(int fd, char *arr, int isize)
{
    ...
    if(((UFDTArr[fd].ptrfiletable->ptrinode->permissiom) != WRITE) && ((UFDTArr[fd].ptrfiletable->ptrinode->permissiom) != READ + WRITE))  return -1;
...
int fstat_file(int fd)
{
    ...
    if(temp->permissiom == 1)
        printf("File Permission: Read only\n");
    else if(temp->permissiom == 2)
        printf("File Permission: Write only\n");
    else if(temp->permissiom == 3)
        ...
        else if(count == 2)
        {
            ...
            else if(strcmp(command[0], "write") == 0)
            {
                ...
                if(ret == -1)
                    printf("ERROR : Incorrect parameter\n");
                if(ret == -2)
                    printf("ERROR : There is no sufficeint memory to write\n");
                if(ret == -3)
